...
public class Comment extends BaseTimeEntity {
    ...
    public static Comment createComment(CommentDTO dto, Post post) {
        ...
        // Entity 생성 및 반환
        return new Comment(dto.getId(), post, dto.getNickname());
    }

    public static Comment createReply(CommentDTO dto, Comment parentComment) {
        if (dto.getId() != null)
            throw new IllegalArgumentException("대댓글 생성 실패 : 대댓글의 id가 없어야 합니다.");

        Post post = parentComment.getPost();

        return new Comment(dto.getId(), post, dto.getContent());
    }
